const navItems = (itemList) => {
  ...
  itemList.forEach((item, index) => {
    navElements.push(<div key={index} className="fw-bold"  id="navItem"><a href="/" className="text-dark" style={{textDecoration: "none"}} active>{item}</a></div>);
  });
    ...
    <>
      <nav className="navbar d-flex align-items-center col-12">
        ...
        <div className="d-flex flex-row col-6 offset-4 justify-content-between">
          ...
          <div>
              <button className="nav-button primary-background text-white fw-bold">Book Ticket</button>
          </div>
        </div>
      </nav>
    </>
